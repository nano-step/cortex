...
#### Filesystem Tools — Nâng cấp toàn diện (từ 4 → 9 tools)
- **`cortex_read_files`** — Batch read tối đa 10 files cùng lúc bằng `Promise.all` (parallel I/O thực sự)
- **`cortex_grep_search`** — Tìm kiếm regex/text qua toàn bộ project, trả về `file:line: match`, tự động skip `node_modules`/`dist`/binary files
- **`cortex_edit_files`** — Batch apply nhiều edits trên nhiều files trong 1 lần gọi, báo cáo `N/M succeeded`
- **`cortex_move_file`** — Rename/move file an toàn, tự tạo parent dirs, chặn overwrite ngẫu nhiên
- **`cortex_delete_file`** — Xóa file/empty directory, sandbox-safe

#### Filesystem Tools — Nâng cấp tools hiện có
- **`cortex_read_file`**: Giới hạn tăng từ 1MB → **10MB**, thêm `offset`+`limit` để đọc file lớn theo chunk
- **`cortex_list_directory`**: Thêm `recursive`, `depth` (1-10), `extensions` filter
- **Progressive fallback cho `cortex_edit_file`**: Khi `old_string` không khớp chính xác:
  - Attempt 1: Exact match
  - Attempt 2: Whitespace-normalized match (trim từng dòng)
  - Attempt 3: Gợi ý line number gần nhất + hướng dẫn dùng `offset+limit` để xem chính xác

#### Unrestricted Mode — Vượt sandbox
- Setting mới: `filesystem_unrestricted_mode` (default: `false`)
- Khi bật: AI có thể đọc/ghi bất kỳ file nào trên máy (kể cả absolute path)
- Vẫn bảo vệ: `/System`, `/etc`, `/bin`, `/sbin`, `/var`, `node_modules`
- Học từ Cline/Roo Code: họ chỉ warn khi ra ngoài workspace, không block

#### Agent Pool — 429 Rate Limit Resilience
- **Dynamic model resolution**: Thay vì hardcoded model names, pool giờ query `getAvailableModels()` để dùng model thực sự available trên proxy
- **Tier-based selection**: `fast` → tier 1-5, `balanced` → tier 5-7, `premium` → tier 8-10; tự động chọn best available trong range
- **Model refresh**: Pool tự refresh model list trước khi execute nếu không có model ready
- **Auto-discover fallback**: Nếu không có model nào trong tier range → dùng bất kỳ model ready nào (best tier first)

### Cải tiến

#### Agent Pool — Retry & Fallback (từ session trước)
- **Exponential backoff với jitter**: `429`/`5xx` → retry sau 1s/2s/4s (±30% jitter)
- **Retry-After header**: Khi server trả về `Retry-After: 30` → chờ đúng 30s (capped 16s)
- **Model tier fallback**: `RESOURCE_EXHAUSTED` trong body → tự chuyển `fast → balanced → premium`
- **Error preservation**: Mọi lỗi retry đều lưu vào `AgentOutput.metadata.errors[]`

### Sửa lỗi
- **Planning skill truncation**: Response `/plan` bị cắt tại 2000 ký tự → tăng lên 8000 ký tự
- **Agent Pool `0/2 succeeded`**: Agents fail ngay khi 429 thay vì retry với backoff
- **Explore/Librarian 429**: Agents dùng hardcoded model names không tồn tại trên proxy
